...
We read out that the number of proteins significant at FDR `r FDR`:  
* At baseline: `r sum(soma.tab$RYGBvsDWMat0mo.FDR < FDR)`  
* At 3 mo in unadj: `r sum(soma.tab$RYGBvsDWMat3mo.FDR < FDR)`; whereas in adj: `r sum(soma.tab$RYGBvsDWMat3mo_bmi.FDR < FDR)`. In both, not inc. lfc, we have `r kable(soma.venn.ss)`  
...
* At 3 mo in unadj: `r sum(met.tab$RYGBvsDWMat3mo.FDR < FDR)`; whereas in adj: `r sum(met.tab$RYGBvsDWMat3mo_bmi.FDR < FDR)`. In both, not inc. lfc, we have `r nrow(met.venn.ss)` metabolites.  
* 3-hydroxyisobutyrate at 3mo: `kable(met.venn.lfc["C1549", c("RYGBvsDWMat3mo_bmi.sig", "RYGBvsDWMat3mo.sig")])`  
