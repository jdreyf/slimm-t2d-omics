...
A table representing a Venn diagram of proteomics using FDR = `r FDR` is at [soma_FDR15_venn.csv](./results/soma_FDR15_venn.csv).

A heatmap of differential proteins is at [soma_heat.pdf](./results/soma_heat.pdf).
...
ezheat(heat.mat, labrows = labRow,  name="soma_FDR15_heat", sc="none", labcols = labCol, 
       reorder_rows = TRUE, reorder_cols = FALSE, fontsize_row = 8, fontsize_col = 16, angle_col = "0")
rownames(heat.mat) <- labRow
write.csv(signif(heat.mat, 3), "../results/soma_heat.csv")
setwd("..")
```
...
We read out that the number of proteins significant:  
* At baseline: `r sum(soma.tab$RYGBvsDWMat0mo.FDR < FDR)`  
* At 3 mo in unadj: `r sum(soma.tab$RYGBvsDWMat3mo.FDR < FDR)`; whereas in adj: `r sum(soma.tab$RYGBvsDWMat3mo_bmi.FDR < FDR)`. In both, we have `r kable(soma.venn.ss)`  
* After baseline correction at any time point without BMI adjustment: `r length(soma.sig.nobmi)`
...
```{r met_venns}
setwd("./results")
met.venn <- ezvenn(met.tab, prefix.v = c(names(contr.v1)[-c(1, 4)], names(contr.bmi)[-3]), fdr.cutoff = 0.15, 
                   name="met_FDR15", plot = FALSE)
...
A table representing a Venn diagram of metabolomics using FDR = 0.15 is at  [met_FDR15_venn.csv](./results/met_FDR15_venn.csv).

```{r metheat}
setwd("./figures")
mets.heat <- ezvenn(met.tab, prefix.v = names(contr.v1)[-c(1, 4)], fdr.cutoff = 0.05, name="met_FDR05", plot = FALSE)
mets.heat <- rownames(mets.heat)[rowSums(abs(mets.heat)) > 0]
heat.mat <- data.matrix(met.stats[mets.heat, grep("logFC$", colnames(met.stats))])
labRow <- sub(";.+", "", met.annot[rownames(heat.mat), "BIOCHEMICAL"])
labCol <- gsub("RYGBvsDWMat|mo\\.logFC", "", colnames(heat.mat))
ezheat(heat.mat, labrows = labRow,  name="met_FDR05_heat", sc="none", labcols = labCol, 
       reorder_rows = TRUE, reorder_cols = FALSE, fontsize_row = 8, fontsize_col = 16, angle_col = "0")
rownames(heat.mat) <- labRow
write.csv(signif(heat.mat, 3), "../results/met_heat.csv")
setwd("..")
```

A heatmap of the `r nrow(heat.mat)` metabolites differential at FDR of 5% (b/c too many at 15%) is at [met_heat.pdf](./results/met_heat.pdf).
...
We read out that the number of mets significant:  
* At baseline: `r sum(met.tab$RYGBvsDWMat0mo.FDR < FDR)`  
* At 3 mo in unadj: `r sum(met.tab$RYGBvsDWMat3mo.FDR < FDR)`; whereas in adj: `r sum(met.tab$RYGBvsDWMat3mo_bmi.FDR < FDR)`. In both, we have `r nrow(met.venn.ss)`: `r kable(met.venn.ss)`  
* After baseline correction at any time point without BMI adjustment: `r length(met.sig.nobmi)`  
* at 3 years w/o BMI: `r sum(abs(met.venn$RYGBvsDWMat36mo.sig) > 0)`  


<!-- # Differential abundance remission at 3 years within RYGB -->


<!-- Analyze each person's analyte difference from baseline at 3mo between those who achieved glycemic endpoints (coded as *dmcure*) at 36mo and those who did not within RYGB. I think we should drop, because doesn’t account for beginning. -->
...
### Compare Hitman to joint significance analyte mediation
...
We apply Hitman to combined (metabolite + protein) dataset, then test pathways.
...
We need to know inter-gene correlation for method `cameraPR`.
...
write.csv(hm.df, "homair12_vs_analytes3_hitman_stats.csv")

hm.df[is.na(hm.df[, "EMY.z"]), "EMY.z"] <- -10**2
cam.homair <- ezcamerapr(stats.tab=hm.df[, "EMY.z", drop=FALSE], G=smpdb_gmt, feat.tab=hm.df, alternative = "greater",
                         inter.gene.cor = igc, name="homair")
setwd("..")
```

We write the hitman results for each analyte's 3mo change from baseline as a mediator of 12mo HOMA-IR change at [homair12_vs_analytes3_hitman_stats.csv](./results/homair12_vs_analytes3_hitman_stats.csv).

## HbA1c

```{r deltas.HbA1c, eval=FALSE}
setwd("./results")
...
hm.df[is.na(hm.df[, "EMY.z"]), "EMY.z"] <- -10**2
cam.a1c  <- ezcamerapr(stats.tab=hm.df[, "EMY.z", drop=FALSE], G=smpdb_gmt, feat.tab=hm.df, alternative = "greater",
                       inter.gene.cor = igc)
setwd("..")
```
...
hm.df[is.na(hm.df[, "EMY.z"]), "EMY.z"] <- -10**2
cam.dins  <- ezcamerapr(gstats=hm.df[, "EMY.z", drop=FALSE], G=smpdb_gmt, feat.tab=hm.df, alternative = "greater",
