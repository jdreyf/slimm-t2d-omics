...
clin3.hm <- Hitman:::lotman(E=arm.v, M=t(diff.pheno3), Y=HbA1c)
write.csv(signif(clin3.hm, 3), "./results/HbA1c12_vs_clin3_lotman.csv", na="")
```
...
pwys <- read.csv("results/RYGBvsDWM_stats_nrot2e+05_roast.csv")
...
We apply Hitman to combined (metabolite + protein) dataset, then test Mesa.
...
res %<-% mesa(M=diff.mat3[,diff.nms], E=exposure.v, Y=diff.pheno12[diff.nms, "homair"],
                     gmat=smpdb_gmat, nperm=nperm, min.nfeats=3, ret.null.mats = TRUE,
...
write.csv(feat.df, paste0("homair_feature_stats_nperm", nperm, "_mesa.csv"))
setwd("..")
```
...
begin <- Sys.time()
c(ps, fs) %<-% mesa(M=diff.mat3[,diff.nms], E=exposure.v, Y=diff.pheno12[diff.nms, "HbA1c"],
...
c(ps, fs) %<-% mesa(M=diff.mat3[,diff.nms], E=exposure.v, Y=diff.pheno12[diff.nms, "dins030"],
                     Gmat=smpdb_gmat, nperm=nperm, min.nfeats=3, 
                     annot.df = hm.df[, c("EMY.p", "EMY.FDR", "CHEBI")], ncores=1, name = nm)
