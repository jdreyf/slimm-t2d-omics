...
met.venn <- ezvenn(met.tab, prefix.v = c(names(contr.v1)[-c(1, 4)], names(contr.bmi)[-3]), fdr.cutoff = FDR, 
                   name=paste0("met_FDR", 100*FDR), plot = FALSE)
...
A table representing a Venn diagram of metabolomics using FDR = `r FDR` is at [met_FDR15_venn.csv](./results/met_FDR15_venn.csv).

```{r met_heat}
setwd("./figures")
mets.heat <- ezvenn(met.tab, prefix.v = names(contr.v1)[-c(1, 4)], fdr.cutoff = FDR, name=paste0("met_FDR", FDR), plot = FALSE)
...
labCol <- gsub("RYGBvsDWMat|mo\\.logFC", "", colnames(heat.mat))
ezheat(heat.mat, labrows = labRow,  name=paste0("met_FDR", 100*FDR,"_heat"), sc="none", labcols = labCol, 
       reorder_rows = TRUE, reorder_cols = FALSE, fontsize_row = 8, fontsize_col = 16, angle_col = "0")
