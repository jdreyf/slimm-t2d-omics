...
The installation of required R packages takes approximately 15 minutes. The RMD file can be knit to Word in approximately 1 minute.
...
Load and attach the above and other R packages.
...
Clinical metadata at [clin_metadata.csv](./data/clin_metadata.csv). This file also used for uploads to GEO. Here, change column names in workflow, so can use them more easily in ggplot2. After taking differences of phenotypes, we remove individual points from R object *diff.pheno*, who have NA in every measure, such as Topaz_3.
...
# rm pathway sort order column
met.annot <- met.annot[, setdiff(colnames(met.annot), "PATHWAY_SORTORDER")]

met.mat.na <- read.csv("data/met_mat_norm_no_impute.csv", row.names=1)
# met.mat.na <- met.mat.na[rowMeans(is.na(met.mat.na)) <= 0.05,]
diff.met.mat.na <- make_diff_mat(met.mat.na)
```
...
The table of numbers in each arm that had HbA1c measurements:

`r kable(table(pheno[!is.na(pheno$HbA1c), c("Mo", "arm")]))`

The table of numbers in each arm that were assayed by SomaLogic:

`r kable(table(pheno[colnames(soma.mat), c("Mo", "arm")]))`

The able of numbers in each arm that were assayed by metabolomics:

`r kable(table(pheno[colnames(met.mat), c("Mo", "arm")]))`
...
dmet12.na <- diff.met.mat.na[,grep("_12$", colnames(diff.met.mat.na))]
```

Calculate metadata differences from baseline and remove samples with too many NAs, who shouldn't be used in Hitman. Write [clin_metadata_diff.csv](./results/clin_metadata_diff.csv) for Table S1. 
...
# write to table s1
diff.clin <- data.frame(pheno[rownames(diff.pheno), c("Acrostic", "arm", "Mo")], diff.pheno)
colnames(diff.clin)[-c(1, 3)] <- clin.names.map[match(sub("^X6", "6", colnames(diff.clin))[-c(1, 3)], 
                                                      sub("/", ".", clin.names.map$abbrev)), "full_name"]
write.csv(diff.clin, "results/clin_metadata_diff.csv", na="")
```
...
diff.pheno18 <- diff.pheno[grep("_18$", rownames(diff.pheno)),]
rownames(diff.pheno18) <- gsub("_18$", "", rownames(diff.pheno18))

diff.pheno24 <- diff.pheno[grep("_24$", rownames(diff.pheno)),]
rownames(diff.pheno24) <- gsub("_24$", "", rownames(diff.pheno24))

diff.pheno36 <- diff.pheno[grep("_36$", rownames(diff.pheno)),]
rownames(diff.pheno36) <- gsub("_36$", "", rownames(diff.pheno36))
```

Test association of metabolite abundance to missingness
...
write.csv(heat.df, "soma_heat.csv")
setwd("..")
```

The heatmap w/ `r nrow(heat.df)` analytes is at [soma_FDR15_fc50pct_heat.pdf](./results/soma_FDR15_fc50pct_heat.pdf).

```{r soma.heat.per.sample, eval=FALSE}
# for reviewer
# soma.tmp <- cbind(soma.mat[soma.sig.lfc.nobmi, grep("_0$", colnames(soma.mat))],
#                   diff.soma.mat[soma.sig.lfc.nobmi, ])

pheno.tmp <- pheno[colnames(soma.mat), c("Mo", "arm")]
pheno.tmp <- pheno.tmp[order(pheno.tmp$Mo, pheno.tmp$arm),]

setwd("figures")
ezheat(object=soma.mat[soma.sig.lfc.nobmi, rownames(pheno.tmp)], pheno.df = pheno.tmp, labrows = labRow, 
       sc = "ctr", name = "soma_heat_per_sample", fontsize_col = 2, reorder_rows = TRUE, clip = 2)
setwd("..")
```
...
met.diff.stats <- limma_contrasts(diff.met.mat, grp=grp[colnames(diff.met.mat)], contr=contr.v1[-c(1, 4)], 
                                  add.means = FALSE)
...
# na
met.stats.na <- limma_contrasts(met.mat.na, grp=grp[colnames(met.mat.na)], contr.v1[-4])
met.diff.stats.na <- limma_contrasts(diff.met.mat.na, grp=grp[colnames(diff.met.mat.na)], contr=contr.v1[-c(1, 4)], 
                                     add.means = FALSE)
met.bmi.diff.stats.na <- signif(limma_contrasts(diff.met.mat.na, design = des.met.diff.bmi[colnames(diff.met.mat.na),], 
                                              contr=contr.bmi[-3], add.means = FALSE), 3)
...
# na
met.tab.na <- data.frame(met.stats.na[rownames(met.diff.stats.na), c(avg.cols, t0.cols)], met.diff.stats.na, 
                      met.bmi.diff.stats.na[rownames(met.diff.stats.na),], met.annot[rownames(met.diff.stats.na),])
write.csv(df_signif(met.tab.na), "./results/met_supp_table_no_impute.csv", na="")

# ntop <- 20
# len.int <- length(intersect(rownames(met.diff.stats)[order(met.diff.stats$RYGBvsDWMat3mo.p)][1:ntop],
#       rownames(met.diff.stats.na)[order(met.diff.stats.na$RYGBvsDWMat3mo.p)][1:ntop]))
```
...
We find that the results from not imputing missing values are very similar to those from imputing. 
...
We summarize that the number of metabolites significant at FDR `r FDR`:  
...
diff.soma.mat3.ss <- data.frame(diff.soma.mat[top.soma, nms.tmp], check.names = FALSE)
...
diff.met.mat3.ss <- data.frame(diff.met.mat[top.met, nms.tmp], check.names = FALSE)
rownames(diff.met.mat3.ss) <- met.annot[rownames(diff.met.mat3.ss), "BIOCHEMICAL"]
```

Cluster proteins and metabolites.

```{r clust}
dsm3 <- t(scale(t(diff.soma.mat3.ss), center = TRUE, scale = TRUE))
dsm3.hc <- hclust(dist(dsm3), method="ward.D2")
diff.soma.mat3.ss <- diff.soma.mat3.ss[dsm3.hc$order, ]

dmm3 <- t(scale(t(diff.met.mat3.ss), center = TRUE, scale = TRUE))
dmm3.hc <- hclust(dist(dmm3), method="ward.D2")
diff.met.mat3.ss <- diff.met.mat3.ss[dmm3.hc$order, ]
```
...
for (soma.row in rownames(diff.soma.mat3.ss)){
  for (met.row in rownames(diff.met.mat3.ss)){
    ct <- cor.test(x=as.numeric(diff.soma.mat3.ss[soma.row,]), y=as.numeric(diff.met.mat3.ss[met.row,]))
...
Find strong correlations to comment upon

```{r}
cor.tab <- t(cor.r.out)
wh <- which(abs(cor.tab) > 0.6, arr.ind = TRUE)
cor.tab.ss <- cor.tab[unique(wh[,1]), unique(wh[,2])]
mets.sss <- c("pro-hydroxy-pro", "beta-hydroxyisovalerate", "isovalerylcarnitine")
prots.sss <- c("IGFBP2", "CNDP1", "GHR")
cor.array[prots.sss, mets.sss, ]
```
...
hm.a1c.met <- hitman(E=as.numeric(diff.pheno12[nms.tmp, "arm"]=="RYGB"), 
                     M=diff.met.mat[,paste0(nms.tmp, "_3")], 
                     Y=diff.pheno12[nms.tmp, "HbA1c"])
...
#met.na
diff.met.mat.na.ss <- diff.met.mat.na[rowMeans(is.na(diff.met.mat.na)) < 0.2,]
nms.tmp <- intersect(gsub("_3$", "", grep("_3$", colnames(diff.met.mat), value=TRUE)), rownames(diff.pheno12))
hm.a1c.met.na <- hitman(E=as.numeric(diff.pheno12[nms.tmp, "arm"]=="RYGB"),
                     M=diff.met.mat.na[,paste0(nms.tmp, "_3")], 
                     Y=diff.pheno12[nms.tmp, "HbA1c"])
hm.a1c.met.na.df <- data.frame(signif(hm.a1c.met.na, 3), met.annot[rownames(hm.a1c.met.na),])
write.csv(hm.a1c.met.na.df, "./results/HbA1c12_vs_met3_hitman_no_impute.csv")
...
We also test protein changes at 3 months vs. HbA1c changes at 24 months, and protein & metabolite changes at 36 months.

```{r hm_a1c_24_36}
## 18 months
diff.pheno18.ss <- diff.pheno18[!is.na(diff.pheno18$HbA1c),]
nms.tmp <- intersect(gsub("_3$", "", grep("_3$", colnames(diff.soma.mat), value=TRUE)), rownames(diff.pheno18.ss))
hm.a1c18.soma <- hitman(E=as.numeric(diff.pheno18[nms.tmp, "arm"]=="RYGB"), 
                  M=diff.soma.mat[,paste0(nms.tmp, "_3")], 
                  Y=diff.pheno18[nms.tmp, "HbA1c"])
hm.a1c18.soma.df <- data.frame(signif(hm.a1c18.soma, 3), soma.annot[rownames(hm.a1c18.soma),])
# write.csv(hm.a1c.soma.df, "./results/HbA1c18_vs_soma3_hitman.csv")

nms.tmp <- intersect(gsub("_3$", "", grep("_3$", colnames(diff.met.mat), value=TRUE)), rownames(diff.pheno18.ss))
hm.a1c18.met <- hitman(E=as.numeric(diff.pheno18[nms.tmp, "arm"]=="RYGB"), 
                     M=diff.met.mat[,paste0(nms.tmp, "_3")], 
                     Y=diff.pheno18[nms.tmp, "HbA1c"])
hm.a1c18.met.df <- data.frame(signif(hm.a1c18.met, 3), met.annot[rownames(hm.a1c18.met),])
# write.csv(hm.a1c.met.df, "./results/HbA1c18_vs_met3_hitman.csv")

## 24 months
diff.pheno24.ss <- diff.pheno24[!is.na(diff.pheno24$HbA1c),]
nms.tmp <- intersect(gsub("_3$", "", grep("_3$", colnames(diff.soma.mat), value=TRUE)), rownames(diff.pheno24.ss))
hm.a1c24.soma <- hitman(E=as.numeric(diff.pheno24[nms.tmp, "arm"]=="RYGB"), 
                  M=diff.soma.mat[,paste0(nms.tmp, "_3")], 
                  Y=diff.pheno24[nms.tmp, "HbA1c"])
hm.a1c24.soma.df <- data.frame(signif(hm.a1c24.soma, 3), soma.annot[rownames(hm.a1c24.soma),])
# write.csv(hm.a1c.soma.df, "./results/HbA1c24_vs_soma3_hitman.csv")

nms.tmp <- intersect(gsub("_3$", "", grep("_3$", colnames(diff.met.mat), value=TRUE)), rownames(diff.pheno24.ss))
hm.a1c24.met <- hitman(E=as.numeric(diff.pheno24[nms.tmp, "arm"]=="RYGB"), 
                     M=diff.met.mat[,paste0(nms.tmp, "_3")], 
                     Y=diff.pheno24[nms.tmp, "HbA1c"])
hm.a1c24.met.df <- data.frame(signif(hm.a1c24.met, 3), met.annot[rownames(hm.a1c24.met),])
# write.csv(hm.a1c.met.df, "./results/HbA1c24_vs_met3_hitman.csv")

## 36 months
diff.pheno36.ss <- diff.pheno36[!is.na(diff.pheno36$HbA1c),]
nms.tmp <- intersect(gsub("_3$", "", grep("_3$", colnames(diff.soma.mat), value=TRUE)), rownames(diff.pheno36.ss))
hm.a1c36.soma <- hitman(E=as.numeric(diff.pheno36[nms.tmp, "arm"]=="RYGB"), 
                  M=diff.soma.mat[,paste0(nms.tmp, "_3")], 
                  Y=diff.pheno36[nms.tmp, "HbA1c"])
hm.a1c36.soma.df <- data.frame(signif(hm.a1c36.soma, 3), soma.annot[rownames(hm.a1c36.soma),])
# write.csv(hm.a1c.soma.df, "./results/HbA1c36_vs_soma3_hitman.csv")

nms.tmp <- intersect(gsub("_3$", "", grep("_3$", colnames(diff.met.mat), value=TRUE)), rownames(diff.pheno36.ss))
hm.a1c36.met <- hitman(E=as.numeric(diff.pheno36[nms.tmp, "arm"]=="RYGB"), 
                     M=diff.met.mat[,paste0(nms.tmp, "_3")], 
                     Y=diff.pheno36[nms.tmp, "HbA1c"])
hm.a1c36.met.df <- data.frame(signif(hm.a1c36.met, 3), met.annot[rownames(hm.a1c36.met),])
# write.csv(hm.a1c.met.df, "./results/HbA1c36_vs_met3_hitman.csv")
```
...
The protein analyte(s) more significant in p-value & FDR than any clinical mediator are: `r kable(hm.soma.top)`

The metabolite mediator(s) are `r hm.a1c.met.df.ss$BIOCHEMICAL`
...
We combine several mediation plots into the Hitman figure using `ggplot2 facet_wrap`. We modify this figure by copy-pasting pieces into PPT, then adding text boxes for Inconsistent and Consistent mediators.
...
hm.df$an <- factor(x=hm.df$an, levels=c('Retinol', 'GHR'), ordered=TRUE)

mu.tbl <- hm.df %>% 
  group_by(Arm, an) %>%
  summarise(mu = mean(x))

ggp <- ggplot(data=hm.df, mapping = aes(x=x, y=a1c, color=Arm)) + facet_wrap(facets = ~an, nrow=1, scales="free_x") + 
  geom_point(alpha=0.7) + geom_smooth(method="lm", se=FALSE) + xlab(paste("Baseline-corrected log2 abundance (3 mo)")) + 
  ylab("HbA1c change (1 yr)") + theme_bw() + theme(legend.position = "none") +
  theme(text=element_text(size=8), axis.text.x=element_text(size=4)) + 
  scale_colour_manual(values=c("orange", "green4")) + 
  geom_vline(mapping=aes(xintercept=mu, color=Arm), data=mu.tbl, linetype="dashed")
ggsave(file="figures/hitman_figure.pdf", plot=ggp, height=5.5, width=19.05, units="cm")
```
...
#met.na
diff.met.mat.na.ss <- diff.met.mat.na[rowMeans(is.na(diff.met.mat.na)) < 0.2,]
hm.homair.met.na <- hitman(E=as.numeric(diff.pheno12[nms.tmp, "arm"]=="DWM"), 
                      M=diff.met.mat.na.ss[,paste0(nms.tmp, "_3")], 
                      Y=diff.pheno12[nms.tmp, "homair"])
hm.met.na.df <- data.frame(signif(hm.homair.met.na, 3), met.annot[rownames(hm.homair.met.na),])
write.csv(hm.met.na.df, paste0("./results/homair12_vs_met3_hitman_no_impute.csv"))
```
...
nms.tmp <- intersect(gsub("_3$", "", grep("_3$", colnames(diff.met.mat), value=TRUE)),
                     rownames(diff.pheno12)[!is.na(diff.pheno12$dins030)])
hm.dins.met <- hitman(E=as.numeric(diff.pheno12[nms.tmp, "arm"]=="DWM"), 
                      M=diff.met.mat[,paste0(nms.tmp, "_3")], 
                      Y=diff.pheno12[nms.tmp, "dins030"])
...
#met.na
diff.met.mat.na.ss <- diff.met.mat.na[rowMeans(is.na(diff.met.mat.na)) < 0.2,]
hm.dins.met.na <- hitman(E=as.numeric(diff.pheno12[nms.tmp, "arm"]=="DWM"), 
                      M=diff.met.mat.na.ss[,paste0(nms.tmp, "_3")], 
                      Y=diff.pheno12[nms.tmp, "dins030"])
hm.met.na.df <- data.frame(signif(hm.dins.met.na, 3), met.annot[rownames(hm.dins.met.na),])
write.csv(hm.met.na.df, paste0("./results/dins030_12_vs_met3_hitman_no_impute.csv"))
```
...
# Dependence matrix for high-throughput simulation

```{r soma_cor_mat}
ngene <- 500
set.seed(seed=1)
gene.ind <- sample(x=nrow(soma.mat), size = ngene)
dsoma3 <- diff.soma.mat[gene.ind, grep(pattern="_3$", x = colnames(diff.soma.mat))]
dsoma3b <- removeBatchEffect(x=dsoma3, batch=diff.pheno3[colnames(dsoma3), "arm"])
cov.mat <- cov(t(dsoma3b))
# write.csv(signif(cov.mat, digits=6), "data/soma_cov_mat_subset.csv")
# median(diag(cov.mat)) #0.163
# mean(sqrt(sqrt(diag(cov.mat)))) #0.65**4 = 0.18
cov.mat.sc <- cov.mat/median(diag(cov.mat))
write.csv(signif(cov.mat.sc, digits=6), "data/soma_cov_mat_subset_scaled.csv")
```
...
#proteome
fdr.t <- c("RYGBvsDWMat3mo.FDR", "RYGBvsDWMat12mo.FDR", "RYGBvsDWMat24mo.FDR", "RYGBvsDWMat36mo.FDR")
bmi.fdr.t <- c("RYGBvsDWMat3mo_bmi.FDR", "RYGBvsDWMat12mo_bmi.FDR", "RYGBvsDWMat24mo_bmi.FDR", "RYGBvsDWMat36mo_bmi.FDR")
lfc.t <- c("RYGBvsDWMat3mo.logFC", "RYGBvsDWMat12mo.logFC", "RYGBvsDWMat24mo.logFC", "RYGBvsDWMat36mo.logFC")
bmi.lfc.t <- c("RYGBvsDWMat3mo_bmi.logFC", "RYGBvsDWMat12mo_bmi.logFC", "RYGBvsDWMat24mo_bmi.logFC", "RYGBvsDWMat36mo_bmi.logFC")
#shbg
stopifnot(soma.tab["SL005102", fdr.t] < 0.15)
stopifnot(soma.tab["SL005102", bmi.fdr.t] > 0.15)
stopifnot(soma.tab["SL005102", lfc.t] > 0)
#adipoq
stopifnot(soma.tab["SL004258", fdr.t][-1] < 0.15)
stopifnot(soma.tab["SL004258", bmi.fdr.t] > 0.15)
stopifnot(soma.tab["SL004258", lfc.t] > 0)
#leptin
stopifnot(soma.tab["SL000498", fdr.t][2:3] < 0.15)
stopifnot(soma.tab["SL000498", bmi.fdr.t] > 0.15)
stopifnot(soma.tab["SL000498", lfc.t] < 0)
#crp
stopifnot(soma.tab["SL000051", fdr.t][c(2,4)] < 0.15)
stopifnot(soma.tab["SL000051", bmi.fdr.t] > 0.15)
stopifnot(soma.tab["SL000051", lfc.t] < 0)
#cndp1
stopifnot(soma.tab["SL006694", fdr.t][1:2] < 0.15)
stopifnot(soma.tab["SL006694", bmi.fdr.t][1] < 0.15)
stopifnot(soma.tab["SL006694", bmi.fdr.t][-1] > 0.15)
stopifnot(soma.tab["SL006694", lfc.t] < 0)
stopifnot(soma.tab["SL006694", bmi.lfc.t] < 0)
#ghr
stopifnot(rownames(soma.tab)[soma.tab$EntrezGeneSymbol == "GHR"] == "SL005168")
stopifnot(soma.tab["SL005168", fdr.t] < 0.15)
stopifnot(soma.tab["SL005168", bmi.fdr.t] > 0.15)
stopifnot(soma.tab["SL005168", lfc.t] < 0)
stopifnot(soma.tab["SL005168", bmi.lfc.t] < 0)
stopifnot(fc2ratio(2**as.numeric(soma.tab["SL005168", lfc.t])) < -.24)
#ibsp
stopifnot(soma.tab["SL004660", fdr.t] < 0.15)
stopifnot(soma.tab["SL004660", bmi.fdr.t][1:2] < 0.15)
stopifnot(soma.tab["SL004660", lfc.t] > 0)
stopifnot(soma.tab["SL004660", bmi.lfc.t] > 0)
#omd
stopifnot(soma.tab["SL008574", fdr.t][-1] < 0.15)
stopifnot(soma.tab["SL008574", bmi.fdr.t][-1] < 0.15)
stopifnot(soma.tab["SL008574", lfc.t] > 0)
stopifnot(soma.tab["SL008574", bmi.lfc.t] > 0)
#igfbp2
stopifnot(soma.tab["SL000466", fdr.t] < 0.15)
stopifnot(soma.tab["SL000466", bmi.fdr.t][1] < 0.15)
stopifnot(soma.tab["SL000466", lfc.t] > 0)
stopifnot(soma.tab["SL000466", bmi.lfc.t] > 0)
#fgf19
stopifnot(soma.tab["SL004337", fdr.t][4] < 0.15)
stopifnot(soma.tab["SL004337", bmi.fdr.t][4] < 0.15)
stopifnot(soma.tab["SL004337", lfc.t] > 0)
stopifnot(soma.tab["SL004337", bmi.lfc.t] > 0)
#cfb
stopifnot(soma.tab["SL000414", fdr.t][-1] < 0.15)
stopifnot(soma.tab["SL000414", bmi.fdr.t][-1] < 0.15)
stopifnot(soma.tab["SL000414", lfc.t] < 0)
stopifnot(soma.tab["SL000414", bmi.lfc.t] < 0)

soma.tmp <- rownames(soma.tab)[soma.tab$EntrezGeneSymbol %in% c("CFB")]
stopifnot(soma.tab[soma.tmp, fdr.t][-1] < 0.15)
stopifnot(soma.tab[soma.tmp, bmi.fdr.t][-1] < 0.15)
stopifnot(soma.tab[soma.tmp, lfc.t] < 0)
stopifnot(soma.tab[soma.tmp, bmi.lfc.t] < 0)
soma.tmp <- rownames(soma.tab)[soma.tab$EntrezGeneSymbol %in% c("RBP4")]
stopifnot(soma.tab[soma.tmp, fdr.t] > 0.15)
stopifnot(soma.tab[soma.tmp, bmi.fdr.t] > 0.15)
stopifnot(soma.tab[soma.tmp, lfc.t][-3] < 0)
stopifnot(soma.tab[soma.tmp, bmi.lfc.t] < 0)
soma.tmp <- rownames(soma.tab)[soma.tab$EntrezGeneSymbol %in% c("IGFBP1", "IGFBP2", "IGFBP3")]
# stopifnot(soma.tab[soma.tmp, fdr.t] > 0.15)
# stopifnot(soma.tab[soma.tmp, bmi.fdr.t] > 0.15)
stopifnot(soma.tab[soma.tmp, lfc.t] > 0)
# stopifnot(soma.tab[soma.tmp, bmi.lfc.t] < 0)
...
#metabolome
fdr.tm <- c("RYGBvsDWMat3mo.FDR", "RYGBvsDWMat12mo.FDR", "RYGBvsDWMat36mo.FDR")
bmi.fdr.tm <- c("RYGBvsDWMat3mo_bmi.FDR", "RYGBvsDWMat12mo_bmi.FDR", "RYGBvsDWMat36mo_bmi.FDR")
lfc.tm <- c("RYGBvsDWMat3mo.logFC", "RYGBvsDWMat12mo.logFC", "RYGBvsDWMat36mo.logFC")
bmi.lfc.tm <- c("RYGBvsDWMat3mo_bmi.logFC", "RYGBvsDWMat12mo_bmi.logFC", "RYGBvsDWMat36mo_bmi.logFC")

met.tmp <- rownames(met.tab)[met.tab$BIOCHEMICAL == "3-hydroxyisobutyrate"]
stopifnot(met.tab[met.tmp, fdr.tm[1]] < 0.15)
stopifnot(met.tab[met.tmp, bmi.fdr.tm[1]] < 0.15)
stopifnot(met.tab[met.tmp, lfc.tm] < 0)
met.tmp <- rownames(met.tab)[met.tab$BIOCHEMICAL == "pro-hydroxy-pro"]
stopifnot(met.tab[met.tmp, fdr.tm] < 0.15)
stopifnot(met.tab[met.tmp, bmi.fdr.tm][1:2] < 0.15)
stopifnot(met.tab[met.tmp, lfc.tm] > 0)
stopifnot(met.tab[met.tmp, bmi.lfc.tm] > 0)
met.tmp <- rownames(met.tab)[met.tab$BIOCHEMICAL == "glycochenodeoxycholate sulfate"]
stopifnot(met.tab[met.tmp, fdr.tm][2] < 0.15)
stopifnot(met.tab[met.tmp, bmi.fdr.tm][2:3] < 0.15)
stopifnot(met.tab[met.tmp, lfc.tm] > 0)
stopifnot(met.tab[met.tmp, bmi.lfc.tm] > 0)
met.tmp <- rownames(met.tab)[met.tab$BIOCHEMICAL == "metformin"]
stopifnot(met.tab[met.tmp, fdr.tm][2] < 0.15)
stopifnot(met.tab[met.tmp, bmi.fdr.tm][2] < 0.15)
stopifnot(met.tab[met.tmp, lfc.tm] < 0)
stopifnot(met.tab[met.tmp, bmi.lfc.tm] < 0)
met.tmp <- rownames(met.tab)[met.tab$BIOCHEMICAL == "choline"]
stopifnot(met.tab[met.tmp, fdr.tm] < 0.15)
stopifnot(met.tab[met.tmp, bmi.fdr.tm][1] < 0.15)
stopifnot(met.tab[met.tmp, lfc.tm] < 0)
stopifnot(met.tab[met.tmp, bmi.lfc.tm] < 0)
met.tmp <- rownames(met.tab)[met.tab$BIOCHEMICAL == "histidine"]
stopifnot(met.tab[met.tmp, fdr.tm][1] < 0.15)
stopifnot(met.tab[met.tmp, bmi.fdr.tm][1] < 0.15)
stopifnot(met.tab[met.tmp, lfc.tm][1] < 0)
stopifnot(met.tab[met.tmp, bmi.lfc.tm][1] < 0)
met.tmp <- rownames(met.tab)[met.tab$BIOCHEMICAL == "retinol (Vitamin A)"]
stopifnot(met.tab[met.tmp, fdr.tm][1] < 0.15)
stopifnot(met.tab[met.tmp, bmi.fdr.tm][1] < 0.15)
stopifnot(met.tab[met.tmp, lfc.tm] < 0)
stopifnot(met.tab[met.tmp, bmi.lfc.tm] < 0)
...
stopifnot(cor.array["GHR", "pro-hydroxy-pro", "p"] < 10**(-5))
stopifnot(cor.array["GHR", "pro-hydroxy-pro", "FDR"] < 0.002)
...
stopifnot(cor(as.numeric(met.mat["C35127",]), as.numeric(met.mat.na["C35127",])) >= 0.99)
```

Explore missingness
```{r, eval=FALSE}
nna <- rowMeans(is.na(met.mat.na))
nna.df <- data.frame(RYGBvsDWMat3mo.p=met.tab.na[, "RYGBvsDWMat3mo.p"], prop.na=nna[rownames(met.tab.na)], met.annot[rownames(met.tab.na),])
nna.df <- nna.df[order(nna.df$RYGBvsDWMat3mo.p),]
```
