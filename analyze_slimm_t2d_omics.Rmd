...
write.csv(soma.venn.df, "output/soma_FDR05_venn.csv")
...
write.csv(met.venn.df, "./output/met_FDR05_venn.csv")
...
setwd("output")
fs <- read.csv("deltadelta_t.csv", row.names = 1)
ps <- read.csv("absdeltadelta_stats_nperm50000_pants.csv", row.names = 1)
#net plots
fs.t <- setNames(fs$RYGBvsDWM.t, nm=rownames(fs))
plot_pwy(gr=gr, ker=ker, Gmat=smpdb_gmat, pwy="Beta-Alanine Metabolism", annot=chebi.map, ntop=10, score.v=fs.t, seed=4)
plot_pwy(gr=gr, ker=ker, Gmat=smpdb_gmat, pwy="Phospholipid Biosynthesis", annot=chebi.map, ntop=7, score.v=fs.t, seed=4)
```
...
stopifnot("HbA1c" %in% colnames(pheno))
stopifnot(diff.pheno["RADIO_3", "HbA1c"] == pheno["RADIO_3", "HbA1c"]-pheno["RADIO_0", "HbA1c"])
stopifnot(pc.pheno["RADIO_3", "BMI"] == (pheno["RADIO_3", "BMI"]-pheno["RADIO_0", "BMI"])/pheno["RADIO_0", "BMI"])
