...
# COLEC11 <- as.numeric(diff.soma.mat["SL006713", paste0(rownames(diff.pheno12), "_3")])
# TIMP1 <- as.numeric(diff.soma.mat["SL000591", paste0(rownames(diff.pheno12), "_3")])
...
hm.df$an <- factor(hm.df$an, levels=c('Retinol', 'GHR'), ordered=TRUE)
ggp <- ggplot(data=hm.df, mapping = aes(x=x, y=a1c, color=Arm)) + facet_wrap(facets="~an", nrow=1, scales="free_x") + geom_point(alpha=0.7) + 
  geom_smooth(method="lm", se=FALSE) + xlab(paste("Baseline-corrected log2 abundance (3 mo)")) + 
  ylab("HbA1c change (1 yr)") + theme_bw() +
  theme(text=element_text(size=7)) + theme(axis.text.x=element_text(size=5)) + scale_colour_manual(values=c("blue", "red"))
ggsave(file="figures/hitman_figure.pdf", plot=ggp, height=3)
```
