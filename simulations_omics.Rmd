...
cov.type <- "CH" # CH for simPATHy::chimera
if (cov.type == "structured"){
  ...
  dataCH <- simPATHy::chimera
  set.seed <- 0
  gene.ind <- sample(x=1:nrow(dataCH), size=ngene)
  sampledata <- t(dataCH[gene.ind,])
  Sigma <- cov(sampledata)
  cov_mat <- Sigma/median(diag(Sigma))
  ...
  foreach(prop = prop.v) %:%
    foreach(sc = sc.v, .packages='Hitman') %dopar% {
      if (sc=="sc1"){
        ...
        sc3 <- Hitman::sim_omics(nsamp=nsamp, ngene=ngene, FDR=fdr, prop.consistent=prop, prop.inconsistent = prop2,
...
sc3 <- data.frame(Method=rep(c("Hitman", "Lotman", "Joint signif consistent", "Joint signif both"), each=length(prop.v)*length(nsamp.v)), 
                  ...
                  N_incons_med=rep(ceiling(prop.v*ngene/2), times=length(nsamp.v)),
