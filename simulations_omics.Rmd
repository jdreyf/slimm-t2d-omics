...
cov.type <- "gtex" # CH for simPATHy::chimera
if (cov.type == "structured"){
  ...
  cov_mat <- as.matrix(read.csv("../data/gtex_muscle_subset_scaled.csv", row.names = 1))
    ...
    foreach(sc = sc.v, .packages='Hitman') %dopar% {
      if (sc=="sc1"){
        ...
        write.csv(sc1, paste0("sim_omics_sc1_nsim", nsim,  "_prop", prop, "_t1.", t1.nm, "_ngene", ngene, "_N", nsamp, 
                              "_b1t2", b1, "_cov", cov.type.out, ".csv"))
        sc1
      } else if (sc=="sc2"){
        t1 <- 5*prop*ngene*b1
        ...
        sc3 <- Hitman::sim_omics(nsamp=nsamp, ngene=ngene, FDR=fdr, prop.consistent=prop, prop.inconsistent = prop,
                                       b1 = b1, t2=t2, t1=t1, nsim=nsim, Sigma = cov_mat, prop.1c = prop.1c)
...
for (nsamp in nsamp.v){
  for (prop in prop.v){
    sc1.tmp <- read.csv(paste0("sim_omics_sc1_nsim", nsim,  "_prop", prop, "_t1.", t1.nm, "_ngene", ngene, 
                               "_N", nsamp, "_b1t2", b1, "_cov", cov.type.out, ".csv"))
...
write.csv(sc1.wr, paste0("combined_sc1_omics_nsim", nsim, "_b1t2", b1, "_cov", cov.type.out, ".csv"), row.names = FALSE)

sc2.wr <- sc2[order(sc2$N, sc2$N_mediators), setdiff(colnames(sc2), "prop")]
write.csv(sc2.wr, paste0("combined_sc2_omics_nsim", nsim, "_b1t2", b1, "_cov", cov.type.out, ".csv"), row.names = FALSE)

sc3.wr <- sc3[order(sc3$N, sc3$N_cons_med), setdiff(colnames(sc3), "prop")]
write.csv(sc3.wr, paste0("combined_sc3_omics_nsim", nsim, "_b1t2", b1, "_cov", cov.type.out, ".csv"), row.names = FALSE)
```
