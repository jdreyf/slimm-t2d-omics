...
sim.m <- Mesa::sim_msa(msea, Gmat=smpdb_gmat, nsample=nsample, nsim=nsim, nperm=nperm, effect.v = effect.v, ncores=1)
...
```{r, eval=FALSE}
sim.gmt <- Mesa::sim_msa(msa.fcn = global_mediation_test, Gmat=smpdb_gmat, nsample=nsample, nsim=nsim, verbose=TRUE, effect.v = effect.v)
write.csv(sim.gmt[1,, drop=FALSE], paste0("./validation/sim_gmt_nperm", nperm, "_nsim", nsim, ".csv"), na="")
```
